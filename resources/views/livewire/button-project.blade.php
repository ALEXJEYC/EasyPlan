            ...
            <div class="flex justify-center mb-4">
                <button wire:click="openModal"
                    ...
                    <div class="text-center transform transition-transform group-hover:scale-110">
                        ...
                        <p class="mt-1 text-base font-medium text-gray-700 dark:text-gray-300">Nuevo Proyecto</p>
                    </div>
                </button>
                ...
                <div class="px-3 py-2 space-y-3">
                    <form wire:submit.prevent="createProject" class="space-y-3">
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                <label
                                    class="block text-xs font-medium text-gray-700 dark:text-gray-200 mb-1">Miembros*</label>
                                <div
                                    class="mt-1 space-y-1 max-h-[120px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
                                    @foreach ($this->organizationMembers as $member)
                                        <div
                                            class="flex items-center p-1 hover:bg-gray-50 dark:hover:bg-gray-700 rounded">
                                            <input type="checkbox" id="member-{{ $member->id }}"
                                                value="{{ $member->id }}" wire:model="members"
                                                class="h-3 w-3 text-blue-600 border-gray-300 dark:border-gray-600" />
                                            <label for="member-{{ $member->id }}"
                                                class="ml-2 text-xs text-gray-700 dark:text-gray-200">{{ $member->name }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                @error('members')
                        ...
                        <div class="pt-3">
                            <button type="submit"
                                ...
                                <svg wire:loading wire:target="createProject" class="animate-spin h-3 w-3"
