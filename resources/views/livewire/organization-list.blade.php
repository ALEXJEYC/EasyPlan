    ...
    @else
        @foreach ($organizations as $organization)
            <a href="{{ route('organizations.show', $organization) }}" class="block group">
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-md overflow-hidden border border-gray-200 dark:border-gray-700 hover:shadow-xl hover:border-indigo-400 transition-all duration-300">
                    <!-- Imagen o iniciales de la organización -->
                    <div class="relative">
@if($organization->hasMedia('images'))
    <img src="{{ $organization->getFirstMediaUrl('images') }}"
...
@endif
                        <span class="absolute top-2 right-2 bg-indigo-100 text-indigo-700 text-xs px-3 py-1 rounded-full font-semibold shadow">
                            ID: {{ $organization->id }}
                        </span>
                    </div>
                    ...
                    <div class="p-5">
                        <h3 class="text-xl font-bold text-gray-800 dark:text-gray-100 mb-1 truncate">{{ $organization->name }}</h3>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ $organization->description ?? 'Sin descripción disponible.' }}</p>
