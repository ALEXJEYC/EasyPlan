...
<head>
    ...
    <link rel="mask-icon" href="/safari-pinned-tab.svg" color="#5bbad5"> <!-- Si existe -->
    ...
    @auth
        <div x-data="{ sidebarOpen: false, mobileMenuOpen: false }" class="flex h-screen relative">
            ...
            <template x-if="sidebarOpen">
                <div class="fixed inset-0 z-10 flex items-start justify-start pt-24 pl-4 pr-8 bg-black/40 backdrop-blur-sm md:hidden"
                    @click.self="sidebarOpen = false" x-show="sidebarOpen"
                    x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
                    x-transient:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
                    x-init="initLogoAnimation()">
                    <div class="bg-white dark:bg-gray-800 w-72 rounded-2xl shadow-2xl p-4" x-show="sidebarOpen"
            ...
            <aside
                class="hidden md:block fixed top-28 left-10 h-[calc(95vh-7rem)] w-64 bg-white dark:bg-gray-800 shadow-lg rounded-xl p-4 z-10">
                <x-layouts.navbarleft />
            </aside>
